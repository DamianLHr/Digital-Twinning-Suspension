    ...
    public class BumpPipeline : MonoBehaviour
    {
        ...
        private void OnDistance(float distance)
        {
            ...
            if (Time.frameCount % 10 == 0)
                Debug.Log($"[BP] state={CurrentState} dist={distance:F4} h={height:F4} trig={triggerHeight:F4} pos={pos:F3}");
            ...
            switch (CurrentState)
            {
                case State.Idle:
                    PushRing(pos, height);
                    if (height >= triggerHeight) BeginBump(pos);
                    break;

                case State.Accumulating:
                    AppendBump(pos, height);
                    if (height < triggerHeight) _flatStreak++; else _flatStreak = 0;
                ...
                case State.Cooldown:
                    PushRing(pos, height);
                    if (height >= triggerHeight) { BeginBump(pos); break; }
