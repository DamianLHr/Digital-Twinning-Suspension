...
namespace Suspension.Sensors
{
    ...
    public class DigitalToFSensor : DigitalSensorBase
    {
        ...
        [Tooltip("Quantization step (m). VL53L0X reports in mm.")]
        [SerializeField] private float resolution = 0.001f;
        ...
        {
            base.Initialize();
            noiseModel.quantization = resolution;
        ...
        protected override void Sample()
        {
            ...
            {
                // Apply bias + Gaussian noise, then clamp into [0, range] and quantize.
                float d = noiseModel.Apply(hit.distance);
                if (d < 0f) d = 0f;
                if (d > range) d = range;
                tofOutput.Publish(d);
